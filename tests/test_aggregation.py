...
def _trajectory_df():
    ...
    return pl.DataFrame({
        ...
        "ts":        [
            datetime(2024, 1, 15, 8, 10),
            datetime(2024, 1, 15, 8, 25),
            datetime(2024, 1, 15, 8, 40),
...
class TestAggregateGpsTable:
    ...
    def test_hour_extracted_correctly(self):
        df, _ = aggregate_gps_table(_trajectory_df(), ["lat", "lon"], "speed", "ts", k=1)
        hours = set(df["hour_of_day"].to_list())
        assert hours == {8, 9}
    ...
    def test_ping_count_correct(self):
        df, _ = aggregate_gps_table(_trajectory_df(), ["lat", "lon"], "speed", "ts", k=1)
        cell_49_6 = df.filter(pl.col("lat") == 49.6)
        assert cell_49_6["ping_count"][0] == 3

    def test_avg_speed_correct(self):
        df, _ = aggregate_gps_table(_trajectory_df(), ["lat", "lon"], "speed", "ts", k=1)
        cell_49_6 = df.filter(pl.col("lat") == 49.6)
        assert cell_49_6["avg_speed_kmh"][0] == pytest.approx((30.0 + 45.0 + 50.0) / 3)
    ...
    def test_two_different_hours_give_two_cells_for_same_location(self):
        df = pl.DataFrame({
            ...
            "ts":    [
                datetime(2024, 1, 15, 8, 0), datetime(2024, 1, 15, 8, 10), datetime(2024, 1, 15, 8, 20),
                datetime(2024, 1, 15, 9, 0), datetime(2024, 1, 15, 9, 10), datetime(2024, 1, 15, 9, 20),
            ],
