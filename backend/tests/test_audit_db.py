...
class TestSchemaInit:
    ...
    def test_expected_ddl_statements(self, mock_psycopg2):
        # 7 CREATE TABLE statements (runs, column_events, alerts, config,
        # column_exclusions, table_targets, apply_row_scan) + 2 ALTER TABLE
        # migrations (_DDL_RUNS_MIGRATIONS: key_vault_key_version, stage_seconds).
        _, _, cur = mock_psycopg2
        AuditDB("postgresql://test")
...
class TestLoadRowScanColumns:
    """apply_row_scan opts (table, column) pairs into the targeted row-by-row
    Presidio scan."""

    def test_creates_apply_row_scan_ddl(self, mock_psycopg2):
        _, _, cur = mock_psycopg2
        AuditDB("postgresql://test")
        sqls = [c.args[0] for c in cur.execute.call_args_list]
        assert any("apply_row_scan" in s for s in sqls)

    def test_returns_empty_when_no_rows(self, mock_psycopg2):
        _, _, cur = mock_psycopg2
        db = AuditDB("postgresql://test")
        cur.execute.reset_mock()
        cur.fetchall.return_value = []

        assert db.load_row_scan_columns() == {}

    def test_groups_columns_by_lowercased_table(self, mock_psycopg2):
        _, _, cur = mock_psycopg2
        db = AuditDB("postgresql://test")
        cur.execute.reset_mock()
        cur.fetchall.return_value = [
        ...
        result = db.load_row_scan_columns()

        assert result == {
            "orders": frozenset({"notes", "description"}),
            "tickets": frozenset({"body"}),
        }

    def test_query_targets_apply_row_scan(self, mock_psycopg2):
        _, _, cur = mock_psycopg2
        db = AuditDB("postgresql://test")
        cur.execute.reset_mock()
        cur.fetchall.return_value = []

        db.load_row_scan_columns()

        sql = cur.execute.call_args.args[0]
        assert "apply_row_scan" in sql
