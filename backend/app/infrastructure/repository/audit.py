...
# Operator-curated (table, column) pairs that should receive the targeted
# row-by-row Presidio scan.  Presence here opts the column into cell-by-cell
# scanning.  This is the "more targeted approach" that replaces the globally-
# disabled row scan — only listed columns are scanned, so the false positives
# that came from scanning every text column are avoided.
_DDL_APPLY_ROW_SCAN = """
CREATE TABLE IF NOT EXISTS apply_row_scan (
    table_name TEXT NOT NULL,
    column_name TEXT NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY (table_name, column_name)
...
class AuditDB:
    ...
    def _init_schema(self) -> None:
        with self._cursor() as cur:
            ...
            cur.execute(_DDL_APPLY_ROW_SCAN)
    ...
    def load_row_scan_columns(self) -> dict[str, frozenset[str]]:
        """Return ``{table_name: frozenset(column_names)}`` from ``apply_row_scan``.

        Each listed (table, column) pair opts that column into the targeted
        row-by-row Presidio scan. Table names are lowercased so lookups are
        case-insensitive.
        """
        result: dict[str, list[str]] = {}
        with self._cursor() as cur:
            cur.execute("SELECT table_name, column_name FROM apply_row_scan")
            for table, column in cur.fetchall():
                result.setdefault(table.lower(), []).append(column)
        return {t: frozenset(cols) for t, cols in result.items()}
