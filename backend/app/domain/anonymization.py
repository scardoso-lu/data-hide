...
def _detect_language(text: str) -> str:
    ...
    langdetect is RNG-based and non-deterministic by default: for short or
    ambiguous strings ``detect()`` returns different languages across runs,
    which would route the same value to different per-language engines and make
    anonymization non-reproducible (a value masked in one run could leak in the
    next). Pinning ``DetectorFactory.seed`` makes routing deterministic, which a
    compliance pipeline requires. The default 'en' fallback below already covers
    any non-supported result, so the seeded language only ever picks among the
    supported set in practice.
    """
    try:
...
def summarize_residual_pii(df: pl.DataFrame) -> tuple[int, str]:
    """Non-raising counterpart to ``validate_residual_pii``.

    Returns ``(total_count, human_readable_detail)`` without aborting. The
    pipeline uses this to RECORD and ALERT on residual PII while still writing
    the (partially-anonymized) table — a single over-eager recognizer (e.g.
    false-positive PHONE_NUMBER hits on a numeric column) must not abort the
    run. ``validate_residual_pii`` is retained for callers/tests that want the
    strict, raising contract.
    """
    residuals = _merge_residual_summaries(residual_pii_findings(df))
    return _residual_total_and_detail(residuals)


def _residual_total_and_detail(residuals: list[dict]) -> tuple[int, str]:
...
def _raise_for_residuals(residuals: list[dict]) -> int:
    total, detail = _residual_total_and_detail(residuals)
    if total:
        raise RuntimeError(
