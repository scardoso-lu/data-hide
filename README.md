...
| Art. 5(1)(c) — Data minimisation | Reduce GPS precision | Round lat/lon to N decimal places (default 1 d.p. ≈ 11 km for city data); floor co-located timestamps to day | `app/classification.py` · `app/anonymization.py` | `detect_gps_columns()` · `detect_timestamp_columns()` · `anonymize_gps_columns()` · `bin_timestamp_columns()` |
...
| `GPS_PRECISION` | No (default `1`) | Decimal places for spatial rounding of GPS columns (1 ≈ 11 km city default, 2 ≈ 1 km for low-density point data) |
