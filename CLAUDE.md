...
- `uv run python -m spacy download en_core_web_lg && uv run python -m spacy download fr_core_news_lg && uv run python -m spacy download de_core_news_lg`: install the required NLP models (English, French, German/Luxembourgish) for full anonymization tests and local runs. The `_lg` defaults are load-bearing for the semantic recognizers and Tier B2 thresholds — an `_md` swap was tried (2026-06-02) and reverted after failing 12 protected detection cases. `SPACY_MODEL_<LANG>=..._md` remains an operator escape hatch (~800 MB RSS savings) for deployments that accept the recall loss.
...
Use Python 3 style with 4-space indentation, type hints where they clarify function contracts, and focused functions with explicit names. Keep constants in uppercase, environment variable names uppercase, and tests named `test_<behavior>.py`. Prefer structured data handling with Polars and existing helper functions over ad hoc parsing — the pipeline is Polars-native end to end (pandas exists only as the presidio-structured boundary inside `classify_pii_columns` Tier B1). Keep runtime behavior stateless; do not add container-local file writes.
...
## PII Detection Test Guarantees

> **2026-06-02 — type lock lifted by user approval.** The previous "frozen contract" forbade any modification to the test classes and fixtures below. The user explicitly authorized the pandas → Polars migration, which required porting DataFrame constructions and accessors in these tests (`pd.DataFrame` → `pl.DataFrame`, `.iloc[i]` → `[i]`, dtype assertions `object` → `pl.String`/`pl.Object`). **The detection guarantees themselves remain protected**: the rules below still apply to the *inputs* and *assertion strength* of every case.

The test classes and parametrize fixtures listed below in `tests/test_anonymization.py` encode regulatory-grade detection guarantees built through 25 TDD iterations covering GDPR Art. 9/10 special categories, B2B/B2C identifiers, and typo tolerance. Each input is a documented regression: it was observed leaking on a previous version of the pipeline, and the assertion is the minimum guarantee that proves the leak was closed.

**Protected test classes** (`tests/test_anonymization.py`):
...
**Protected parametrize fixtures** (module-level lists in the same file):
...
- **Adding is allowed; subtracting is not.** New entries in any protected fixture, and new test classes for new PII categories, are encouraged. Use a new fixture/class rather than modifying an existing entry.
- **If a protected test fails, fix the recognizer — not the test.** A failure here means a recognizer in `app/domain/recognizers.py` or a setting in `app/domain/anonymization.py` was changed in a way that re-introduced a leak (or a false positive). Restore the recognizer behaviour or add a new pattern; do not edit the input or the assertion.
- **Treat `PRESIDIO_SCORE_THRESHOLD` and `GDPR_ENTITIES` in `app/domain/anonymization.py` as part of this contract.** Raising the threshold, or removing an entity type from the list, must be accompanied by a full `uv run pytest` pass — many context-required recognizers (NATIONAL_TAX_ID, BOOKING_REF, POSTAL_CODE, CONTRACT_NUMBER, SWIFT_BIC, DATE_OF_BIRTH, SALARY) rely on the current threshold for their score boost to clear.
- **Column-policy layer** in `app/domain/classification.py` (`classify_pii_columns`, `apply_column_policies`, `CONCEPT_SEEDS`, `DEFAULT_ENTITY_ACTIONS`, `PURVIEW_TYPE_TO_ENTITY`) is part of this contract. The three classification tiers (Purview → presidio-structured → spaCy embedding similarity) are load-bearing — removing a tier means leaking known regressions like the `first_name` / `Jimmy` case. New env vars `ENABLE_PRESIDIO_STRUCTURED` (default on), `ENABLE_COLUMN_POLICY` (default on), and `COLUMN_SIMILARITY_THRESHOLD` (default 0.55) are operator escape hatches; do not change their defaults without re-running the full suite.
- **A truly-incorrect protected case can be amended only with explicit user approval**, in a PR whose description names the case and explains why the previous expectation was wrong. Do not silently rewrite.
- **DataFrame types are NOT part of the protection.** The suite is Polars-native; further type-level refactors of the test scaffolding are ordinary changes, provided inputs and assertion strength are preserved.
